...
- The Lambda's only outside call is the Overpass API: 15-second timeout (Lambda 20 s), responses over 3 MB rejected, a `User-Agent` naming the app. At most one fetch per card; after that, S3 serves it. After a failed call, the Lambda waits 30 s before calling Overpass again (its usage policy).
