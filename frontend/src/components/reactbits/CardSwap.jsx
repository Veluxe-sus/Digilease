// React Bits <CardSwap />. https://reactbits.dev
// DigiLease edits: the upstream `.card` class is renamed `.swap-card` (`.card` is far
// too generic to drop into an app that already has cards), and the rotation stops
// under prefers-reduced-motion instead of spinning forever.
...
const CardSwap = ({
  ...
  useEffect(() => {
    ...
    // One card can't swap with anything, and a viewer who asked for less motion
    // should get a still stack rather than a carousel that never stops.
    const still = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    if (total < 2 || still) return undefined;
